...
/** Домен Смерти - краткие формулировки механики 2014, не полный текст правил */
export function domainSpellIdsForLevel(level: number): string[] {
  return DEATH_DOMAIN_SPELLS.filter((e) => e.characterLevel <= level).flatMap(
    (e) => e.spellIds,
  )
}
...
export const DEATH_DOMAIN_FEATURES: DomainFeature[] = [
  {
    id: 'death_reaper',
    name: 'Жнец',
    level: 1,
